...
<div class="wrap smm-wrap">
    <div class="smm-header">
        <div>
            <h1><?php esc_html_e( 'Simple Maintenance Mode', 'simple-maintenance-mode' ); ?></h1>
            <p><?php esc_html_e( 'Lightweight maintenance and coming-soon pages without page-builder bloat.', 'simple-maintenance-mode' ); ?></p>
        </div>
        <span class="smm-version">v<?php echo esc_html( SMM_VERSION ); ?></span>
    </div>
        ...
        <div class="smm-grid">
            <main>
                <section class="smm-card">
                    <h2><?php esc_html_e( 'Site Status', 'simple-maintenance-mode' ); ?></h2>
                    <div class="smm-status-options">
                        <?php foreach ( array( 'online' => 'Online', 'coming_soon' => 'Coming Soon', 'maintenance' => 'Maintenance' ) as $key => $label ) : ?>
                            <label class="smm-radio-card"><input type="radio" name="status" value="<?php echo esc_attr( $key ); ?>" <?php checked( $s['status'], $key ); ?>><span><?php echo esc_html( $label ); ?></span></label>
                        <?php endforeach; ?>
                    </div>
                    <p class="description"><?php esc_html_e( 'Administrators always retain access.', 'simple-maintenance-mode' ); ?></p>
                </section>

                <section class="smm-card">
                    <h2><?php esc_html_e( 'Page & Search Engine Response', 'simple-maintenance-mode' ); ?></h2>
                ...
                <section class="smm-card">
                    <h2><?php esc_html_e( 'Layout', 'simple-maintenance-mode' ); ?></h2>
                    <div class="smm-layout-grid">
                        <?php foreach ( array( 'centered' => 'Centered', 'centered-card' => 'Centered Card', 'split-left' => 'Split Left', 'split-right' => 'Split Right', 'bottom-panel' => 'Bottom Panel', 'minimal' => 'Minimal' ) as $key => $label ) : ?>
                            <label class="smm-layout-option"><input type="radio" name="layout" value="<?php echo esc_attr( $key ); ?>" <?php checked( $s['layout'], $key ); ?>><span class="smm-layout-demo smm-layout-<?php echo esc_attr( $key ); ?>"></span><strong><?php echo esc_html( $label ); ?></strong></label>
                        <?php endforeach; ?>
                    </div>
                </section>

                <section class="smm-card">
                    <h2><?php esc_html_e( 'Background', 'simple-maintenance-mode' ); ?></h2>
                ...
                <section class="smm-card">
                    <h2><?php esc_html_e( 'Content & Branding', 'simple-maintenance-mode' ); ?></h2>
                ...
                <section class="smm-card">
                    <h2><?php esc_html_e( 'Container Styling', 'simple-maintenance-mode' ); ?></h2>
                ...
                <section class="smm-card">
                    <h2><?php esc_html_e( 'Optional Elements', 'simple-maintenance-mode' ); ?></h2>
                    <label class="smm-toggle"><input type="checkbox" name="show_countdown" value="1" <?php checked( $s['show_countdown'], 1 ); ?>><span>Show countdown</span></label>
                    <div class="smm-field smm-countdown-field"><label>Launch date</label><input type="datetime-local" name="countdown_date" value="<?php echo esc_attr( $s['countdown_date'] ); ?>"></div>
                    <hr>
                    <label class="smm-toggle"><input type="checkbox" name="show_button" value="1" <?php checked( $s['show_button'], 1 ); ?>><span>Show call-to-action button</span></label>
                    <div class="smm-button-fields"><div class="smm-field"><label>Button label</label><input class="regular-text" name="button_label" value="<?php echo esc_attr( $s['button_label'] ); ?>"></div><div class="smm-field"><label>Button URL</label><input class="regular-text" type="url" name="button_url" value="<?php echo esc_attr( $s['button_url'] ); ?>"></div><label><input type="checkbox" name="button_new_tab" value="1" <?php checked( $s['button_new_tab'], 1 ); ?>> Open in a new tab</label><div class="smm-inline-fields"><div><label>Button color</label><input class="smm-color" name="button_bg_color" value="<?php echo esc_attr( $s['button_bg_color'] ); ?>"></div><div><label>Button text</label><input class="smm-color" name="button_text_color" value="<?php echo esc_attr( $s['button_text_color'] ); ?>"></div></div></div>
                </section>

                <section class="smm-card">
                    <h2><?php esc_html_e( 'Access & URL Exemptions', 'simple-maintenance-mode' ); ?></h2>
                    <label class="smm-toggle"><input type="checkbox" name="bypass_enabled" value="1" <?php checked( $s['bypass_enabled'], 1 ); ?>><span>Enable secure bypass links</span></label>
                    <div class="smm-field"><label>Bypass duration</label><div><input type="number" name="bypass_duration" min="1" max="168" value="<?php echo esc_attr( $s['bypass_duration'] ); ?>"> hours</div></div>
                    <div class="smm-bypass-box"><code id="smm-bypass-url"><?php echo esc_html( $bypass_url ); ?></code><button type="button" class="button" id="smm-copy-bypass">Copy</button></div>
                    <label><input type="checkbox" name="regenerate_token" value="1"> Generate a new bypass token when saving</label>
                    <hr>

                    <h3><?php esc_html_e( 'Custom Login URL', 'simple-maintenance-mode' ); ?></h3>
                    <div class="smm-field">
                        <label for="custom_login_url"><?php esc_html_e( 'Login path', 'simple-maintenance-mode' ); ?></label>
                        <div>
                            <input class="regular-text code" type="text" id="custom_login_url" name="custom_login_url" value="<?php echo esc_attr( $s['custom_login_url'] ); ?>" placeholder="/login/">
                            <p class="description"><?php esc_html_e( 'If a security plugin hides the standard WordPress login page, enter the replacement login path here. The normal /wp-login.php remains allowed automatically.', 'simple-maintenance-mode' ); ?></p>
                        </div>
                    </div>

                    <h3><?php esc_html_e( 'Additional Exempt URLs', 'simple-maintenance-mode' ); ?></h3>
                    <div class="smm-field">
                        <label for="exempt_urls"><?php esc_html_e( 'Public paths', 'simple-maintenance-mode' ); ?></label>
                        <div>
                            <textarea class="large-text code" id="exempt_urls" name="exempt_urls" rows="6" placeholder="/status/&#10;/payment-callback/&#10;/webhooks/*"><?php echo esc_textarea( $s['exempt_urls'] ); ?></textarea>
                            <p class="description"><?php esc_html_e( 'One same-site path per line. Exact matching is used by default. Add * at the end to allow that path and everything below it, for example /webhooks/*. Query strings are ignored. External domains are rejected.', 'simple-maintenance-mode' ); ?></p>
                            <p class="description"><strong><?php esc_html_e( 'Security note:', 'simple-maintenance-mode' ); ?></strong> <?php esc_html_e( 'Only exempt URLs that genuinely need to remain public. Exempted routes bypass the maintenance screen for everyone.', 'simple-maintenance-mode' ); ?></p>
                        </div>
                    </div>
                    <hr>

                    <label class="smm-toggle"><input type="checkbox" name="block_rest_api" value="1" <?php checked( $s['block_rest_api'], 1 ); ?>><span>Block public REST API access while active</span></label>
                    <label class="smm-toggle"><input type="checkbox" name="block_xmlrpc" value="1" <?php checked( $s['block_xmlrpc'], 1 ); ?>><span>Disable XML-RPC while active</span></label>
                </section>
            </main>

            <aside>
                <section class="smm-card smm-sticky">
                    <h2>Preview & Save</h2>
                    <p>Save your settings, then preview the public maintenance page.</p>
                    <?php submit_button( 'Save Settings', 'primary large', 'submit', false ); ?>
                    <a class="button button-secondary button-large" target="_blank" rel="noopener" href="<?php echo esc_url( add_query_arg( 'smm_preview', wp_create_nonce( 'smm_preview' ), home_url( '/' ) ) ); ?>">Preview Page</a>
                    <div class="smm-status-summary"><strong>Current status</strong><span class="smm-pill smm-pill-<?php echo esc_attr( $s['status'] ); ?>"><?php echo esc_html( ucwords( str_replace( '_', ' ', $s['status'] ) ) ); ?></span></div>
                </section>
            </aside>
        </div>
    </form>
